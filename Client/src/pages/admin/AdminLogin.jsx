...
const AdminLogin = () => {
  ...
  const [formData, setFormData] = useState({ email: "admin@wedding.com", password: "admin123" });
      ...
      <div className="relative grid w-full max-w-6xl overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/[0.07] shadow-2xl shadow-black/30 backdrop-blur-2xl lg:grid-cols-[1fr_0.9fr]">
          ...
          <div className="relative flex h-full flex-col justify-between">
            ...
            <div className="grid grid-cols-3 gap-3">
              {["Secure", "Responsive", "Organized"].map((item) => (
                <div key={item} className="rounded-[1.4rem] border border-white/10 bg-white/10 p-4 backdrop-blur-xl">
        ...
        <div className="bg-white p-5 text-slate-950 sm:p-8 lg:p-10">
          <div className="mx-auto flex min-h-[560px] max-w-md flex-col justify-center">
            ...
            <p className="mt-8 text-xs font-black uppercase tracking-[0.28em] text-indigo-600">Admin access</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight text-slate-950">Welcome back</h2>
            <p className="mt-3 text-sm font-semibold leading-6 text-slate-500">Login to manage your complete wedding invitation application.</p>

            <form onSubmit={handleSubmit} className="mt-8 space-y-4">
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase tracking-[0.16em] text-slate-500">Email</span>
              ...
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase tracking-[0.16em] text-slate-500">Password</span>
                <div className="relative">
              ...
              {errorMessage && (
                <p className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700">{errorMessage}</p>
              )}
